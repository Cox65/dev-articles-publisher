...
export const replaceCodeLanguage = (
  markdownContent: string,
  languagesMapping: Record<string, string>
) =>
  Object.entries(languagesMapping).reduce(
    (result, [initialLanguage, finalLanguage]) => {
      const regex = new RegExp(`\`\`\`( )*${initialLanguage}`, 'g')
      return result.replaceAll(regex, `\`\`\` ${finalLanguage}`)
    },
    markdownContent
  )

export const removeComments = (markdown: string) => {}
