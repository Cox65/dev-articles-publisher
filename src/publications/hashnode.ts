...
export class HashnodePublication extends Publication {
  ...
  constructor({
    ...
    super({ article, id, url, useAsMain, platform: 'hashnode' })
    ...
    const response =
      await this.fetchHashnodeApi<CreatePublicationStoryResponse>(query, {
        ...
        input: {
          ...
          coverImageURL: this.article.coverImage,
          isPartOfPublication: {
            publicationId: this.publicationId
          },
          contentMarkdown: this.transformedContent,
          tags: [],
          ...(this.article.canonicalUrl
            ? {
                isRepublished: {
                  originalArticleURL: this.article.canonicalUrl
                }
              }
            : {})
        }
