...
export class DevToPublication extends Publication {
  ...
  constructor({
    ...
    super({ article, id, url, useAsMain, platform: 'devTo' })
    ...
    const response = await this.fetchDevToApi<ArticleResponse>('articles', {
      ...
      body: JSON.stringify({
        article: {
          ...
          body_markdown: this.transformedContent,
          tags: this.article.tags,
          main_image: this.article.coverImage
        }
      })
    })
    return {
  ...
  private async fetchDevToApi<T>(
    ...
    if (response.ok) {
      ...
      throw new Error('DevTo publication failed.' + response.statusText)
    }
  }
}
